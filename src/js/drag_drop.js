//.dragがドラッグされている際の動き
	...
	$('.drag').draggable({
    	...
    	$('.drop').draggable({
        	connectToSortable : '.smain_create',
      		//helper: 'clone',
        	helper : function() {
            	return $(this).clone().addClass('drop');
        	},
				...
				$(document).ready(function(){
					...
					$('.drop').droppable({
	  					...
	  					drop: function(ev, ui) {
        					...
        					$("*[name=" + data + "] > div > *[name=culum1]").attr('name' ,"b" + (data + 1));
        					$("*[name=" + data + "] > div > *[name=culum2]").attr('name' ,"b" + (data + 2));
        					$("*[name=" + data + "] > div > *[name=num1]").attr('name',"b"+ (data + 3));
        					$("*[name=" + data + "] > div > *[name=type]").attr('name',"b"+ (data + 4));
        					$("*[name=" + data + "] > div > *[name=num2]").attr('name',"b"+ (data + 5));
        					var limit_name ="a"+ (limit_count[data] + data);
        					$("*[name=" + data + "] > div > *[name=limit]").attr('name',limit_name);
        					if($("*[name="+limit_name+"]").length > 0){
								limit_count[data]++;
